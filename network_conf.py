...
def lstm_net(prev_hidden,
    ...
    S_t = fluid.layers.embedding(input=[pre_word], size=[word_dict_dim, emb_dim])
    x_t = fluid.layers.fc(input=[S_t], size=lstm_him_dim)

    hidden, cell = lstm_cell(x_t=x_t,
...
def lstm_cell(x_t,
    ...
    ## input gate
    i_tx = fluid.layers.fc(input=[x_t],
                                 size=lstm_him_dim,
                                 act=None)
    i_th = fluid.layers.fc(input=[hidden_t_prev],
                                 size=lstm_him_dim,
                                 act=None)
    ...
    ## forget gate
    f_tx = fluid.layers.fc(input=[x_t],
                                 size=lstm_him_dim,
                                 act=None)
    f_th = fluid.layers.fc(input=[hidden_t_prev],
                                 size=lstm_him_dim,
                                 act=None)
    ...
    ## lstm cell
    c_tx = fluid.layers.fc(input=[x_t],
                           size=lstm_him_dim,
                           act=None)
    c_th = fluid.layers.fc(input=[hidden_t_prev],
                           size=lstm_him_dim,
                           act=None)
    ...
    ## output gate
    o_tx = fluid.layers.fc(input=[x_t],
                                 size=lstm_him_dim,
                                 act=None)
    o_th = fluid.layers.fc(input=[hidden_t_prev],
                                 size=lstm_him_dim,
                                 act=None)
...
def lstm_main(prev_hidden,
    ...
    prev_hidden, prev_cell = lstm_net(prev_hidden=prev_hidden,
                                      prev_cell=prev_cell,
    ...
    avg_cost, acc, prediction = lstm_prediction(prev_hidden, word_dict_dim, word)
