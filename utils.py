...
def parse_train_cmd():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help=("The path of img2sent dictionary (default: None). If this parameter "
              "is not set, flickr30k-images will be used. If this parameter "
              "is set, but the file does not exist, word dictionay "
              "will be built from the training data automatically."),
        default=None)
...
def load_default_data():
    ...
    if (not (os.path.exists(tar_img_filename)) or md5file(tar_img_filename) != md5img):
        if os.path.exists(tar_img_filename):
            os.remove(tar_img_filename)
        os.system("wget -O %s -c %s" % (tar_img_filename, img_url))

    if (not (os.path.exists(tar_token_filename)) or md5file(tar_token_filename) != md5token):
...
def get_default_img_feat_path():
    default_data_dir = "./data/"
    default_img_feat_train_path = os.path.join(default_data_dir,
                                          "img_vgg_feats.dict")#"./data/img_vgg_feats.dict"
    default_img_feat_test_path = os.path.join(default_data_dir,
